...
import { load, actions } from '../../../src/routes/admin/foro/hilos/[threadId]/+page.server';
  ...
  const from = vi.fn((table: string) => {
    const b: Record<string, unknown> = {
      ...
      then: (res: (...a: unknown[]) => void, rej: (...a: unknown[]) => void) => {
        const list = table === 'thread_permissions' ? fixture.threadPerms ?? [] : null;
...
const makeEvent = (locals: ReturnType<typeof makeLocals>, body = '', threadId = 't1', url = 'http://localhost/admin/foro/hilos/t1') =>
  ({
    locals,
    params: { threadId },
...
describe('admin/foro/hilos/[threadId] lock/unlock (REQ-FORUM-04.3)', () => {
  it('blocks the author from locking their own thread even when GM', async () => {
    const supabase = makeSupabase({ thread: { id: 't1', author_id: 'gm-1', is_locked: false } });
    await expectError(() => act('lock')(makeEvent(makeLocals(supabase, 'gm', 'gm-1'))), 403);
    expect(supabase.calls.rpc).not.toContainEqual(expect.objectContaining({ name: 'log_audit' }));
  });

  it('blocks a non-staff (rolero) from locking', async () => {
    const supabase = makeSupabase({ thread: { id: 't1', author_id: 'other', is_locked: false } });
    await expectError(() => act('lock')(makeEvent(makeLocals(supabase, 'rolero', 'role-1'))), 403);
  });

  it('lets a GM lock another thread and logs bloquear_hilo', async () => {
    const supabase = makeSupabase({ thread: { id: 't1', author_id: 'other', is_locked: false } });
    const res = (await act('lock')(makeEvent(makeLocals(supabase, 'gm', 'gm-1')))) as { success: boolean };
    expect(res.success).toBe(true);
    expect((supabase.calls.update[0] as { is_locked: boolean }).is_locked).toBe(true);
    expect((supabase.calls.update[0] as { locked_by: string }).locked_by).toBe('gm-1');
    const audit = supabase.calls.rpc.find((r) => (r as { name: string }).name === 'log_audit') as { args: { p_action: string } };
    expect(audit.args.p_action).toBe('bloquear_hilo');
  });

  it('lets an admin reopen a locked thread and logs desbloquear_hilo', async () => {
    const supabase = makeSupabase({ thread: { id: 't1', author_id: 'other', is_locked: true } });
    const res = (await act('unlock')(makeEvent(makeLocals(supabase, 'admin', 'admin-1')))) as { success: boolean };
    expect(res.success).toBe(true);
    expect((supabase.calls.update[0] as { is_locked: boolean }).is_locked).toBe(false);
    const audit = supabase.calls.rpc.find((r) => (r as { name: string }).name === 'log_audit') as { args: { p_action: string } };
...
describe('admin/foro/hilos/[threadId] thread permissions (REQ-FORUM-04.2/04.4)', () => {
  it('persists thread flags and logs editar_permisos', async () => {
    ...
    const res = (await act('setThreadPermissions')(
      makeEvent(makeLocals(supabase, 'admin', 'admin-1'), 'role=rolero&can_view=on&can_post=on'),
    )) as { success: boolean };
    expect(res.success).toBe(true);
    ...
    const audit = supabase.calls.rpc.find((r) => (r as { name: string }).name === 'log_audit') as { args: { p_action: string; p_entity_id: string } };
...
describe('admin/foro/hilos/[threadId] load()', () => {
  it('returns thread and thread permissions for staff', async () => {
    const supabase = makeSupabase({ thread: { id: 't1', author_id: 'other', is_locked: false }, threadPerms: [] });
    const result = (await loadFn(makeEvent(makeLocals(supabase)))) as { thread: { id: string } };
